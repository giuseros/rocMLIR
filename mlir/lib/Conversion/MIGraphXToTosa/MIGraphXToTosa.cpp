...
public:
  ...
  matchAndRewrite(ConvType op,
    ...
    input_t = cast<TypedValue<TensorType>>(getRank4TransposeOp(loc, input_t, rewriter, NCHW2NHWC).getResult());
    filter_t = cast<TypedValue<TensorType>>(getRank4TransposeOp(loc, filter_t, rewriter, NCHW2NHWC).getResult());
...
public:
  ...
  matchAndRewrite(migraphx::ReduceMeanOp op, OpAdaptor adaptor,
    ...
    tosa::ConstOp tosaConstantNumElements =
        createNumElementsTosaConst(loc, cast<TypedValue<TensorType>>(op.getInput()), axis, rewriter);
    auto tosaReciprocal = createOpAndInfer<tosa::ReciprocalOp>(
